...
backup_dir="/etc/greetd/backup-$(date +%Y%m%d-%H%M%S)-regreet-official"
...
for file in /etc/greetd/config.toml /etc/greetd/regreet.toml /etc/greetd/regreet.css /etc/greetd/hyprland.conf; do
    if [[ -f "${file}" ]]; then
...
install -m 644 "${script_dir}/regreet-hyprland.conf" /etc/greetd/hyprland.conf
...
echo "Applied official ReGreet + Hyprland setup."
echo "Backup: ${backup_dir}"
echo "Wallpaper copied to: ${wallpaper_dst}"
echo "Reboot to test the new greeter."